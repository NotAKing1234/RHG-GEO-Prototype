...
const VIEWS = [
  ...
  { id: "recommendations", label: "Proposals", icon: FileText },
  ...
  async function saveRegistrySelection(mode) {
    ...
    try {
      const payload = mode === "checked"
        ...
        : { filters: registryFilters, confirm_large_selection: true };
          ...
          <div className="readiness-facts">
            ...
            <div className="readiness-fact fact-crawl">
              ...
              <span className="readiness-fact-label">
                ...
                <FactInfo label="403 / restricted reads" text={readinessNotation(metrics)} />
              </span>
            </div>
      ...
      <div className="panel registry-control-panel">
        ...
        {data?.registry_missing && (
          <div className="inline-notice warning">
            ...
            <span>
              Full crawl registry is not loaded because {data.registry_path} is missing. Showing distinct URLs from {data.registry_source_path}.
            </span>
          </div>
        )}
...
function RecommendationDetail({ runId, recommendation, data, lookups, saveOverride, busy }) {
  if (!recommendation) {
    return (
  ...
  return (
    <aside className="detail-column pinned">
      ...
      <TicketDraftingPanel runId={runId} recommendation={recommendation} copyBlocks={copyBlocks} saveOverride={saveOverride} busy={busy} />
...
function TicketDraftingPanel({ runId, recommendation, copyBlocks, saveOverride, busy }) {
  ...
  const [jiraExportBusy, setJiraExportBusy] = useState("");
  ...
  useEffect(() => {
    ...
    setAcceptanceCriteriaText(numberedListText(preferredAcceptanceCriteria(existingDraft.acceptance_criteria, defaults.acceptanceCriteria)));
  ...
  async function loadProposalJiraCsv() {
    const payload = await api.get(dashboardExportPath(runId, "jira_csv", recommendation.proposal_id));
    return payload.content || "";
  }

  async function copyProposalJiraCsv() {
    if (!runId || !recommendation.proposal_id) return;
    setJiraExportBusy("copy");
    setJiraExportError("");
    try {
      copyText(await loadProposalJiraCsv());
    } catch (error) {
      setJiraExportError(error.message || String(error));
    } finally {
      setJiraExportBusy("");
    }
  }

  async function downloadProposalJiraCsv() {
    if (!runId || !recommendation.proposal_id) return;
    setJiraExportBusy("download");
    setJiraExportError("");
    try {
      const content = await loadProposalJiraCsv();
      downloadTextFile(content, exportFileName(runId, "jira_csv", recommendation.proposal_id), "text/csv;charset=utf-8");
    } catch (error) {
      setJiraExportError(error.message || String(error));
    } finally {
      setJiraExportBusy("");
  ...
  return (
    <div className="panel ticket-panel">
      ...
      <div className="button-row">
        ...
        <button className="secondary-button" onClick={copyProposalJiraCsv} disabled={!runId || jiraExportBusy === "copy"}>
          {jiraExportBusy === "copy" ? <Loader2 className="spin" size={15} /> : <Clipboard size={15} />}
          <span>Copy Jira CSV</span>
        </button>
        <button className="secondary-button" onClick={downloadProposalJiraCsv} disabled={!runId || jiraExportBusy === "download"}>
          {jiraExportBusy === "download" ? <Loader2 className="spin" size={15} /> : <Download size={15} />}
          <span>Download Jira CSV</span>
        </button>
      </div>
      {jiraExportError && <p className="empty-text">{jiraExportError}</p>}
...
function ExportsView({ runId, preferredExportType = "clipboard" }) {
  ...
  async function loadExport(type = exportType) {
    ...
    setBusy(type);
    try {
      const payload = await api.get(dashboardExportPath(runId, type));
  ...
  function downloadExport() {
    if (!content) return;
    const mimeType = exportType.includes("csv") ? "text/csv;charset=utf-8" : "text/plain;charset=utf-8";
    downloadTextFile(content, exportFileName(runId, exportType), mimeType);
  }
...
function buildTicketDraftDefaults(recommendation, copyBlocks) {
  const ticket = recommendation.jira_ticket || {};
  ...
  const acceptanceCriteria = preferredAcceptanceCriteria(
    ticket.acceptance_criteria,
    buildAcceptanceCriteria(recommendation, proposalType, targetUrls, targetComponent, proposedValue)
  );
  const summaryBase = recommendation.title || `${targetComponent} ${recommendation.proposed_change || "update"}`;
  const generatedSummary = summaryBase.startsWith("GEO -") ? summaryBase : `GEO - ${summaryBase}`;
  const summary = ticket.summary || generatedSummary;
  ...
  const description = ticket.description || generatedDescription;
...
function shortTicketText(value, maxLength = 220) {
  const text = String(value || "").replace(/\s+/g, " ").trim();
  if (text.length <= maxLength) return text;
  return `${text.slice(0, maxLength - 3).trim()}...`;
}

function buildAcceptanceCriteria(recommendation, proposalType, targetUrls, targetComponent, proposedValue) {
  const targetText = targetUrls.join(", ") || (recommendation.page_refs || []).join(", ") || "the listed target URL or page set";
  const component = acceptanceComponentLabel(recommendation, proposalType, targetComponent);
  const requiredChange = shortTicketText(proposedValue || recommendation.proposed_change || "the required change described in the ticket");
  const criteria = [
    `${component} is updated for ${shortTicketText(targetText, 260)}.`,
    `Implemented output matches the required change: ${requiredChange}`
  ];
  const validationText = `${recommendation.title || ""} ${recommendation.proposed_change || ""} ${component}`.toLowerCase();
  if (proposalType === "html_visibility") {
    if (/cloudflare|crawler|robots\.txt|403|waf/.test(validationText)) {
      criteria.push("robots.txt and approved AI retrieval crawler requests validate without 403, WAF challenge, or access-restricted responses.");
    } else if (validationText.includes("llms.txt")) {
      criteria.push("/llms.txt returns HTTP 200 and contains the approved Radisson content manifest.");
    } else {
      criteria.push("Target content is present in server-delivered HTML when JavaScript is disabled.");
    }
  } else if (proposalType === "schema_update") {
    criteria.push("Updated JSON-LD or structured data validates without parsing errors and includes the fields named in the ticket.");
  } else if (proposalType === "metadata_update") {
    criteria.push("Updated head metadata is present in delivered HTML and matches the ticket copy exactly.");
  } else {
    criteria.push("Updated visible content is present in the expected page location and matches the ticket intent.");
  }
  return [
    ...criteria,
    "Existing visible behavior remains unchanged unless the ticket explicitly specifies a visible change.",
    "Validation evidence is attached before the ticket moves to Done."
  ];
}

function acceptanceComponentLabel(recommendation, proposalType, targetComponent) {
  if (targetComponent && targetComponent !== recommendation.surface) return targetComponent;
  const surface = String(recommendation.surface || "").toLowerCase();
  if (surface.includes("strategic") || proposalType === "html_visibility") return "Platform";
  if (proposalType === "schema_update") return "SEO / Schema";
  if (proposalType === "metadata_update") return "SEO / Metadata";
  if (surface.includes("content")) return "Content";
  return targetComponent || "Target component";
}
...
function ticketListItems(value) {
  if (Array.isArray(value)) return value.filter(Boolean);
  const text = String(value || "").trim();
  if (!text) return [];
  if (text.startsWith("- ")) {
    return text.replace(/^- /, "").split(/\s+-\s+/).map((item) => item.trim()).filter(Boolean);
  }
  return numberedTextToList(text);
}

const LEGACY_GENERATED_ACCEPTANCE_CRITERIA = new Set([
...
function legacyGeneratedAcceptanceCriteria(value) {
  const items = ticketListItems(value).map((item) => shortTicketText(item, 1000).toLowerCase());
  return items.length === 4 && items.every((item) => LEGACY_GENERATED_ACCEPTANCE_CRITERIA.has(item));
}

function preferredAcceptanceCriteria(...candidates) {
  for (const candidate of candidates) {
    if (candidate && !legacyGeneratedAcceptanceCriteria(candidate)) {
      return ticketListItems(candidate);
    }
  }
  return [];
}
...
function hasSelectorWarning(rec) {
  return String(rec?.selector_status || "").toLowerCase().startsWith("warning");
}
...
function dashboardExportPath(runId, exportType, proposalId = "") {
  const params = new URLSearchParams({ run_id: runId || "", type: exportType });
  if (proposalId) params.set("proposal_id", proposalId);
  return `/api/dashboard/export?${params.toString()}`;
}

function exportFileName(runId, exportType, proposalId = "") {
  const extension = exportType.includes("csv") ? "csv" : exportType === "json" ? "json" : "txt";
  const base = [runId || "geo", proposalId].filter(Boolean).join("-");
  return `${base}-${exportType}.${extension}`;
}
...
function usePersistentObject(key, defaultValue) {
  const [value, setRawValue] = useState(() => {
    try {
      return normalizePersistentObject(JSON.parse(window.localStorage.getItem(key)), defaultValue);
    } catch {
      return normalizePersistentObject(null, defaultValue);
    }
  });
  const setValue = (nextValue) => {
    setRawValue((current) => {
      const resolved = typeof nextValue === "function" ? nextValue(current) : nextValue;
      return normalizePersistentObject(resolved, defaultValue);
    });
  };
  useEffect(() => {
...
function normalizePersistentObject(candidate, defaultValue) {
